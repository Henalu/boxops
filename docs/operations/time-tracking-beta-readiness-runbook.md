...
| Fichaje automatico web por planificacion | Listo tecnicamente como generacion idempotente desde bloques/asignaciones activos y, desde el corte 2026-05-23, desde `staff_work_windows`, siempre con `source = schedule_auto` y snapshot minimo. | Validar con planificacion real y activar el job DB de franjas si aplica. No prueba presencia real y debe seguir corregible. |
...
| Jornada prevista | Disponible en `staff_work_windows`; puede aportar comparacion prudente para posibles excesos y alimentar fichaje automatico si `scheduleAutoPunchesEnabled` esta activo. | Validar que nadie la interpreta como contrato, payroll ni prueba definitiva de presencia; los punches generados declaran `presenceVerified = false`. |
