...
Decision CG.0 2026-06-28: se separa "IA propia dentro de BoxOps" del "conector ChatGPT operativo". La IA documental/RAG sigue subordinada a documentos/programacion utiles y va al final, pero el conector ChatGPT puede empezar como fase diferenciadora temprana si solo expone herramientas server-side seguras de BoxOps. Primeros casos permitidos: responder informacion operativa rapida desde horario, centros, tipos y asignaciones (`quien da la clase del martes de 9:00 a 11:15`), listar horarios del dia/semana y preparar borradores de plantillas por rango/centro/hora. Las mutaciones quedan para fases posteriores con resumen, confirmacion humana, idempotencia y auditoria `source = chatgpt_connector`. ChatGPT no accede directamente a Supabase, no recibe `service_role`, no ejecuta SQL libre y no toca documentos sensibles, fichaje, payroll, firmas, ubicacion, RRHH sensible ni aprobaciones automaticas.
...
8. Conector ChatGPT operativo si se busca diferenciacion comercial temprana: contrato CG.0 ya fijado en `docs/architecture/chatgpt-connector-contract.md`; despues lectura, borradores y acciones confirmadas.
9. Nativo/push/geofencing si hay razon comercial.
10. IA documental como ultimo extra futuro.
