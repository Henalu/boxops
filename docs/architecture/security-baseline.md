...
OD.1/I.32 anade el gate documental de operativa diaria en `docs/operations/daily-operations-beta-readiness-runbook.md`. Antes de llamar beta interna a un tenant, confirmar por rol que Horario, Plantillas, Asignaciones, Cobertura, Solicitudes, Ausencias, Eventos, Jornada prevista e Inicio funcionan como flujos operativos y que sus evidencias no contienen datos sensibles innecesarios. Cobertura, ausencias y eventos son contexto operativo: no deben mutar horario, aprobar cobertura, aprobar horas extra, generar payroll ni tomar decisiones por IA automaticamente. Jornada prevista puede alimentar fichaje automatico solo mediante el flujo `schedule_auto` controlado, sin probar presencia real.
...
El corte 2026-05-15 implementa `staff_work_windows` como planificacion de presencia prevista del personal dentro de `/app/schedule`. Es contexto operativo, no bloque, no asignacion real y no dato payroll. Desde el corte 2026-05-23 puede generar fichajes `schedule_auto` si `scheduleAutoPunchesEnabled` esta activo; esos punches declaran `presenceVerified = false`, no usan ubicacion y siguen siendo corregibles.
