...
Preview 2026-09-07: el build de la PR #2 termina, pero `/login` devuelve 500 por `Missing Supabase environment variables.`. El login de produccion devuelve 200. Falta revisar/configurar `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` en Preview y redesplegar; la edicion queda pendiente de sesion autenticada en Vercel. Las pruebas UI aprobadas son locales; no se considera validada la preview por su build verde. Diagnostico en `docs/operations/2026-09-07-guardrail-recheck.md`.
