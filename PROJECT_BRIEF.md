...
## Relacion Con BoxWod Y Hub

Decision 2026-06-29: BoxOps y BoxWod son apps separadas dentro del mismo hub. Comparten el Supabase del hub para Auth, organizaciones, centros, perfiles base/personas y acceso tenant, pero no mezclan datos, permisos ni superficies de producto.

Reglas:

- Un coach, owner o admin que ya existe en BoxOps no debe crear un segundo perfil para usar BoxWod.
- BoxOps mantiene la operativa interna: horarios, cobertura, fichaje, documentos, staff y configuracion operativa.
- BoxWod mantiene la experiencia atleta: reservas, waitlist, WOD, resultados, perfil deportivo, progreso y comunidad basica.
- BoxWod va incluido para tenants BoxOps en MVP; app access/billing separado queda futuro.
- `organization_memberships.role` incluye roles BoxOps y `athlete`.
- `is_org_member` sigue siendo acceso operativo BoxOps y excluye usuarios solo `athlete`.
- `is_hub_member` representa membership activa del hub e incluye `athlete`.
- BoxWod consume clases/sesiones derivadas de BoxOps mediante `boxwod_class_sessions`, pero no debe editar directamente `schedule_blocks` desde la experiencia atleta.
- Cualquier ajuste futuro en `organization_memberships`, `person_profiles`, `centers` o Console debe revisar impacto en BoxWod antes de migrar.

## Estado Actual

Validacion remota 2026-09-09: aplicada y verificada en Supabase del hub la migracion `20260907081226_atomic_schedule_template_week` (73/73). Pasan las 35 aserciones SQL remotas con rollback y los recorridos autenticados en Vercel Preview con seis perfiles sinteticos: owner, admin, manager, coach, atleta y owner de otra organizacion. Se comprueban aplicacion/reintento/reemplazo de plantillas, rollback ante solape, asignacion movil y denegaciones de permisos/tenant. Cuentas bloqueadas y sesiones revocadas al terminar; organizaciones QA inactivas. Los avisos de seguridad preexistentes no cambian. Evidencia y limites: `docs/operations/2026-09-09-remote-ui-validation.md`. Main y la prueba HTTPS compartida con BoxWod siguen pendientes; los parrafos del 7 de septiembre reflejan el estado anterior.

Preview 2026-09-07: resuelto el login 500 tras habilitar `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` en Preview desde Vercel. Se conserva el Supabase del hub y sus valores publicos; las claves privilegiadas siguen limitadas a Produccion. El redespliegue `dpl_AoucztbJdyWcqcyipr6LWPXWRRpF` de la PR #2 pasa la comprobacion real de login/recuperacion (200), rechazo de credenciales ficticias y redireccion anonima de `/app` y `/console` (307), sin errores de funcion durante el recorrido. Los 34 casos por perfiles siguen siendo evidencia local. Supabase remoto tiene 72 de las 73 migraciones del repositorio: falta solamente `20260907081226_atomic_schedule_template_week`, que debe aplicarse y verificarse antes de integrar en `main`. Evidencia y limites en `docs/operations/2026-09-07-guardrail-recheck.md`.

Comprobaciones 2026-09-07: corregidos los 22 fallos anteriores de pruebas desactualizadas y diez casos adicionales encontrados al ampliar la revision. Pasan 208 comprobaciones de codigo/helpers y redireccion anonima de Console, 14 casos documentales/paneles y 35 aserciones SQL; TypeScript y ESLint quedan limpios. La repeticion de perfiles espera expresamente el cierre de soporte y se ejecuta en serie con las pruebas documentales, que usan las mismas cuentas. Detalle: `docs/operations/2026-09-07-guardrail-recheck.md`. El usuario confirma que BoxWod aun no esta publicado ni tiene dominio; su prueba HTTPS compartida queda para cuando exista ese destino.

Actualizacion UI 2026-09-07: rama `codex/ajustes-validacion-ui` y PR #2 con pruebas locales autenticadas para owner/admin/manager/coach, atleta, otro tenant y soporte. `npm run test:ui:local` usa fixtures sinteticos separados; la migracion atomica queda activa solo en Supabase local para estos recorridos. La validacion detecta y corrige la precarga de enlaces documentales (accesos auditados sin clic) y el estado confuso de Documentos en modo soporte. Evidencia, comandos y limites: `docs/operations/2026-09-07-ui-validation.md`. Produccion y la sesion HTTPS compartida con BoxWod siguen pendientes de validacion en su entorno destino; integrar en `main` activa Vercel.

Actualizacion 2026-09-07: primer corte de ajustes tras revision del proyecto. Next.js y eslint-config-next pasan a 16.3.4, PostCSS a una version corregida y shadcn queda como herramienta de desarrollo; el lockfile actualizado pasa la auditoria de dependencias. La aplicacion de una plantilla a una semana usa ahora `apply_schedule_template_week`, una RPC con permisos de invocador/RLS que agrupa reemplazo, altas de bloques y asignaciones en una transaccion. Mantiene roles operativos y soporte activo, certificaciones, solapes, vigencia y protecciones de referencias BoxWod. `npm run test:db:template-atomic` prueba la migracion candidata y fixtures sinteticos contra Postgres local con rollback completo. La migracion `20260907081226_atomic_schedule_template_week.sql` debe aplicarse al entorno destino antes de desplegar el nuevo codigo; esta revision no la aplica a servicios remotos. La atomicidad es por semana, no por todo un rango de semanas ni por la sincronizacion de ediciones de plantillas existentes. Detalle y orden de entrega: `docs/operations/2026-09-07-security-and-template-adjustments.md`.
...
- Decision 2026-06-29: BoxOps y BoxWod seran apps separadas dentro del mismo hub y compartiran Supabase para Auth, organizaciones, centros, perfiles base/personas y acceso tenant. Compartir Supabase no convierte BoxWod en modulo interno de BoxOps ni convierte roles BoxOps en permisos BoxWod. Hace falta contrato explicito de app access/billing para activar BoxWod por tenant y conservar datos/permisos de producto separados.
- Decision 2026-06-28: se abre roadmap faseado para un conector ChatGPT operativo separado de la IA propia/documental futura. ChatGPT podra actuar como interfaz externa del usuario para consultar informacion operativa rapida y preparar acciones acotadas, siempre mediante herramientas server-side de BoxOps que revalidan sesion, tenant, rol, permisos, IDs y estado. Primeros casos: responder quien imparte una clase en una fecha/hora, listar horarios de dia/semana y preparar/crear borradores de plantillas por rango, centro, dias y horas. Las mutaciones reales quedan para fases posteriores con resumen, confirmacion humana, idempotencia y auditoria `source = chatgpt_connector`. Quedan prohibidos acceso directo a Supabase, SQL libre, `SUPABASE_SERVICE_ROLE_KEY`, Storage privado, documentos sensibles, fichaje, payroll, firmas, ubicacion, RRHH sensible, aprobaciones automaticas, decisiones de cobertura y entrenamiento/fine-tuning con datos privados del tenant.
- Actualizacion 2026-06-28 CG.1 / 2026-06-29 CG.2A-CG.2B / 2026-06-30 CG.3A-CG.4C: existe una capa interna server-side para el conector en `src/lib/chatgpt-connector-tools.ts` y `src/lib/chatgpt-connector-core.ts`, mas packaging MCP/JSON-RPC interno en `src/lib/chatgpt-connector-mcp.ts` y `/api/chatgpt/mcp`. CG.1 deja lecturas read-only (`list_centers`, `list_class_types`, `get_schedule_for_day`, `get_schedule_at_time` y `get_my_schedule`); CG.2A anade `preview_schedule_template` como previsualizacion read-only/no-mutation de plantillas, con `preview_id` deterministico no persistido, limite de 62 dias/500 bloques, muestra de 20 bloques y warnings; CG.2B anade `create_schedule_template_draft`, que revalida el payload completo, recalcula el `preview_id`, aplica idempotencia por metadata minimizada y crea borradores reales solo en `schedule_templates`/`schedule_template_blocks`; CG.3A anade `prepare_schedule_template_application`, que revalida plantilla/centro/rango/bloques/tipos/coaches, calcula un plan de aplicacion sin mutar horario real y persiste una confirmacion interna con token hasheado, expiracion, `plan_hash`, snapshot minimizado e idempotencia; CG.3B anade `apply_schedule_template`, que revalida token/plan/sesion/tenant/permiso, aplica mediante RPC transaccional, crea `schedule_blocks`/`schedule_block_assignments`, salta duplicados, rechaza conflictos y audita `source = chatgpt_connector`; CG.4A expone discovery, `initialize`, `ping`, `tools/list` y `tools/call`; CG.4B anade OAuth 2.1 + PKCE, protected resource metadata, authorization server metadata, access tokens opacos scoped, expiracion y revocacion y queda probado en ChatGPT/dev mode con `BoxOps QA`; CG.4C anade refresh tokens opacos rotados de 30 dias, revocacion por cadena y credencial Supabase renovable cifrada para mantener RLS sin `service_role`. No hay GPT Action ni UI grande del conector. CG.4 comercial/demo final no debe considerarse cerrado hasta probar lectura, borrador, preparacion, aplicacion confirmada y refresh automatico con evidencia redacted. `get_my_schedule` solo responde si usuario -> persona visible activa -> ficha coach activa se resuelve de forma unica; `class_types` devuelve `default_duration_minutes = null` hasta que exista ese campo real.
...
- Mapa de cierre 2026-05-17/2026-06-28: beta operativa exige datos reales validados, entorno real/staging, Auth/email, purga, credenciales E2E, smokes por rol y runbooks; OD.1/I.32 anade el checklist especifico de operativa diaria completa para beta interna y F.15 anade el checklist especifico de fichaje web/cierre laboral prudente. Webapp v1 vendible exige tenant/permisos, operativa diaria, fichaje web prudente, documentos/firma/certificaciones, hardening produccion, BoxOps Console, onboarding SaaS, billing por fases con Stripe y exportes. El conector ChatGPT operativo queda como diferenciador temprano faseable tras MVP/operativa, empezando por lectura y borradores; la IA documental propia queda como ultimo extra futuro, posterior a una webapp vendible y a documentos/programacion/permisos/auditoria/legal cerrados.
...
- `organization_memberships`: relacion usuario-organizacion con rol/acceso operativo actual de BoxOps; si el hub activa BoxWod, no debe convertirse sin mas en rol universal de ambos productos.
...
| IA propia fuera de las primeras fases | I.26 confirma que sin documentos/programacion utiles, permisos, auditoria y privacidad clara seria decoracion cara y riesgo operativo. El conector ChatGPT operativo queda separado: puede empezar antes como herramientas externas acotadas sobre horario/plantillas, sin IA documental ni decisiones automaticas. |
...
9. Mantener Fase I por cortes seguros: cambios/cobertura ya tienen workflow minimo y trazabilidad operativa I.25, ausencias tienen bandeja e impacto derivado, I.19 deja eventos/festivos/competiciones visibles solo como contexto minimo en Horario, I.24 deja horas extra con revision visible y deteccion operativa prudente de candidatos de posible exceso, I.26 deja IA documental subordinada a documentos/programacion utiles sin implementacion real, E.6/I.27-E.16 avanzan programacion/documentos utiles como consulta autorizada, evidencia prudente y handoff QA/staging, OD.1/I.32 cierra documentalmente operativa diaria para beta interna, y CG.0-CG.4 quedan como conector ChatGPT operativo separado para lectura, borradores y acciones confirmadas. CG.0 fija el contrato en `docs/architecture/chatgpt-connector-contract.md`; CG.1 deja lectura operativa interna read-only; CG.2A-CG.2B deja preview y borradores reales; CG.3A prepara aplicacion confirmable sin mutar horario real; CG.3B aplica plantillas con revalidacion de token, idempotencia, RPC transaccional y auditoria; CG.4A deja packaging MCP interno; CG.4B deja account linking OAuth/token scoped probado en ChatGPT; CG.4C deja refresh tokens rotados con expiracion/revocacion/metadata; el siguiente corte real es demo controlada completa en ChatGPT con lectura, borrador, preparacion, aplicacion confirmada y refresh automatico. Sin automatismos legales, calculo definitivo, payroll, resolucion automatica de cobertura ni decisiones operativas por IA.
...
11. Usar `docs/product/webapp-completion-roadmap.md` como mapa de cierre: primero beta operativa, despues webapp v1 vendible, luego conector ChatGPT operativo si se busca diferenciacion comercial temprana, despues nativo/geofencing si el negocio lo exige y por ultimo IA documental como extra futuro.
...
- `docs/product/webapp-completion-roadmap.md`: mapa de cierre hacia beta operativa, webapp v1 vendible, conector ChatGPT operativo faseado, futuro opcional e IA documental como ultimo extra.
...
- `docs/architecture/hub-and-boxwod-integration.md`: decision de hub compartido con BoxWod, Supabase comun, fronteras de producto y pendientes de app access.
- `docs/architecture/chatgpt-connector-contract.md`: contrato CG.0 y decisiones CG.1 del conector ChatGPT operativo, herramientas, payloads, permisos, errores, auditoria, privacidad, confirmacion e idempotencia.
