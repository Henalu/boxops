...
- Jornada prevista del personal en `/app/schedule`:
  ...
  - no es `schedule_blocks`, no es `schedule_block_assignments`, no es fichaje ni payroll
- Roles MVP aplicados:
...
- Mapa de cierre 2026-05-17: beta operativa exige datos reales validados, entorno real/staging, Auth/email, purga, credenciales E2E, smokes por rol y runbooks; OD.1/I.32 anade el checklist especifico de operativa diaria completa para beta interna y F.15 anade el checklist especifico de fichaje web/cierre laboral prudente. Webapp v1 vendible exige tenant/permisos, operativa diaria, fichaje web prudente, documentos/firma/certificaciones, hardening produccion, onboarding SaaS y exportes. IA queda como ultimo extra futuro, posterior a una webapp vendible y a documentos/programacion/permisos/auditoria/legal cerrados.
...
| Jornada prevista separada del horario | `staff_work_windows` aporta contexto de presencia prevista sin crear clases, asignaciones, fichajes ni restricciones duras sobre cobertura. |
...
10. Usar `docs/product/webapp-completion-roadmap.md` como mapa de cierre: primero beta operativa, despues webapp v1 vendible, luego nativo/geofencing si el negocio lo exige y por ultimo IA como extra futuro.
