...
- Revision 2026-05-27: Console abre el primer modo soporte auditado desde `/console/organizations/[organizationId]` hacia `/app` con motivo breve, caducidad 30/60/120 minutos, auditoria `support_started`/`support_ended` e indicador visible. Solo `platform_owner` o `support` activos pueden usarlo sobre organizaciones `trialing`/`active`; no suplanta usuarios, no crea memberships permanentes, no usa `service_role` y no concede lectura a documentos, fichaje, payroll, firmas ni RRHH sensible. El control manual de acceso mantiene `trialing`/`active` como estados resolubles y bloquea `suspended`/`inactive`.
